...
export default function DuzenleYonetimUyesiPage() {
  const router = useRouter()
  const params = useParams()
  const id = params.id as string
  
  // URL'den group parametresini al
  const getGroupFromUrl = () => {
    if (typeof window !== 'undefined') {
      const path = window.location.pathname
      if (path.includes('merkez-yonetim-kurulu')) return 'merkez-yonetim-kurulu'
      if (path.includes('merkez-denetleme-kurulu')) return 'merkez-denetleme-kurulu'
      if (path.includes('merkez-disiplin-kurulu')) return 'merkez-disiplin-kurulu'
  ...
  console.log('🚀 DuzenleYonetimUyesiPage component yüklendi, ID:', id, 'Group:', currentGroup)
  ...
  const [formData, setFormData] = useState<ManagementMember>({
    ...
    education: '',
    order: 999,
    group: currentGroup // Bu dinamik olacak
  })
  ...
  // Yeni üye için otomatik sıralama
  useEffect(() => {
    if (id === 'yeni') {
  ...
  const loadMember = async () => {
    try {
      ...
      const response = await fetch(`/api/boards/${currentGroup}/${id}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          setFormData({
            ...
            group: memberData.group || currentGroup
          })
          if (memberData.photo) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id === 'yeni' ? `/api/boards/${currentGroup}` : `/api/boards/${currentGroup}/${id}`
      ...
      if (response.ok && result.success) {
        ...
        router.push(`/admin/${currentGroup}`)
      } else {
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold">
              {id === 'yeni' ? 'Yeni Yönetim Kurulu Üyesi' : 'Üye Düzenle'}
            </h1>
          </div>
