...
export async function GET() {
  try {
    ...
    // Merkez denetleme kurulu üyelerini getir
    console.log('🔍 Merkez denetleme kurulu üyeleri aranıyor...')
    const members = await Management.find({ group: 'merkez-denetleme-kurulu' })
    ...
    return NextResponse.json({
      success: true,
      message: 'Merkez denetleme kurulu verileri başarıyla getirildi',
      data: members
    })
  } catch (error) {
    console.error('❌ Merkez denetleme kurulu verileri getirilemedi:', error)
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📝 Yeni merkez denetleme kurulu üyesi verisi:', { name, position, bio, photo, email, phone, experience, education, order })
    ...
    // Yeni merkez denetleme kurulu üyesi oluştur
    ...
    console.log('✅ Yeni merkez denetleme kurulu üyesi kaydedildi:', newMember)
    
    return NextResponse.json({
      success: true,
      message: 'Merkez denetleme kurulu üyesi eklendi',
      data: newMember
    })
  } catch (error) {
    console.error('❌ Merkez denetleme kurulu üyesi eklenemedi:', error)
    return NextResponse.json(
      { 
        success: false, 
