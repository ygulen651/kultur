...
export default function KamuArDetail({ params }: PageProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch(`/api/kamu-ar/${resolvedParams.slug}`)
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {it.images && it.images.length > 0 && (
              <div className="mt-12 pt-8 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="text-center mb-8">
                  <div className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 px-6 py-3 rounded-full border border-blue-200 dark:border-blue-700">
                  ...
                  {it.images.map((image: string, index: number) => (
                    <div key={index} className="group cursor-pointer" onClick={() => openModal(index)}>
                      <div className="relative aspect-square rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:scale-105">
                        ...
                        {/* Görsel numarası */}
                        <div className="absolute top-3 left-3 bg-blue-600 text-white text-sm font-bold px-3 py-1.5 rounded-full shadow-lg transform scale-100 group-hover:scale-110 transition-transform duration-300">
                          {index + 1}
                        </div>
                        ...
                        {/* Hover efekti - border */}
                        <div className="absolute inset-0 border-2 border-transparent group-hover:border-blue-500 transition-colors duration-300 rounded-2xl"></div>
